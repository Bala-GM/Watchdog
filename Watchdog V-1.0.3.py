...
class SettingsDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        layout.addWidget(QLabel("File Move Delay (minutes):"))
    ...
    def load_settings(self):
        ...
        if 'SETTINGS' in config:
            ...
            self.move_delay_input.setValue(config['SETTINGS'].getint('move_delay', 10))  # Default delay to 10 minutes
...
class MonitorApp(QMainWindow):
    ...
    def initUI(self):
        ...
        self.move_delay_minutes = 10  # Default delay for file movement in minutes
    ...
    def load_settings(self):
        ...
        if 'SETTINGS' in config:
            ...
            self.move_delay_minutes = config['SETTINGS'].getint('move_delay', 10)  # Load file move delay
    ...
    def on_file_dropped(self, file_path):
        ...
        config = ConfigParser()
        config.read('settings.ini')
        delay_minutes = config['SETTINGS'].getint('move_delay', 10)
        time.sleep(delay_minutes * 60)  # Convert delay to seconds
        ...
        if self.move_folder_path:
            try:
                ...
                self.log_event_move(f"File moved successfully after {delay_minutes} minute(s).", new_file_path)
            except Exception as e:
