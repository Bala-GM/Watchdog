...
class MonitorApp(QMainWindow):
    ...
    def start_monitoring(self):
        ...
        if not self.folder_path:
            QMessageBox.warning(self, 'Error', 'Please configure the folder path in settings.')
            return
    ...
    def on_created(self, event):
        ...
        self.file_dropped_signal.emit(event.src_path)
    ...
    def send_email_notification(self):
        ...
        send_email(
            ...
            to_recipients=self.email_to,
            cc_recipients=self.email_cc,
            attachment_paths=[self.log_file_path] if self.log_file_path else None
        )

    def log_event(self, message):
        """Logs events to the specified log file."""
